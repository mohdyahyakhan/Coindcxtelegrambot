# COINDEX V8.8.12 - EMA SMA9 + TP No #2 + RUNNER -1% TRAIL + ST EXIT - BOT3 SAME
...
async def start_command(u,c):
    await u.message.reply_text("✅ Bot v8.8.12 | EMA300 SMA9 | TP No #2 | Runner -1% Trail ST Exit")
@authorized_only
...
async def watchlist_command(u,c):
    ...
    async with _lock:
        ...
        msg+=f"\n📊 NSE ORB V8.8.12: {len(ORB_LEVELS)}/7 stocks EOD 3:00-3:10 PM\n"
        if ORB_LEVELS:
...
async def pnl3_command(u,c):
    ...
    await u.message.reply_text(f"🇮🇳 BOT3 PNL V8.8.12 SAME\nBalance: ₹{b['total_balance']:.2f}\nPnL: {b['lifetime_pnl_percent']:.2f}%")
@authorized_only
...
@authorized_only
async def help_command(u,c): await u.message.reply_text("V8.8.12 EMA300+SMA9 | TP No #2 | Runner -1% Trail + ST Exit")
...
async def nseorb_command(u,c):
    global ORB_LEVELS
    await u.message.reply_text("⏳ NSE ORB V8.8.12... 7 WINNERS stocks")
    ...
    msg = f"📊 NSE ORB READY V8.8.12 {count}/{len(STOCKS_NSE)}\n"
    if ORB_LEVELS:
        for k,v in ORB_LEVELS.items(): msg+=f"{k} H:{v['high']:.1f} L:{v['low']:.1f}\n"
    if failed: msg+=f"\nFailed: {','.join(failed)}"
    await u.message.reply_text(msg)
    ...
    try:
        ...
        if clow <= trade['tp']:
            if attempt == 1 and not trade.get('tp1_hit'):
                ...
                asyncio.create_task(send_telegram(client, f"✅ <b>TP -10% HIT #{attempt} CLOSED - No #2 Watchlist Clear</b> {symbol}")); return

        # 3) RUNNER -1% TRAIL + ST BULLISH EXIT
            ...
            try:
                ...
                if st_dir == -1 or live_close > st_line:
                    ...
                    async with _lock:
                        ...
                        WATCHLIST.pop(symbol, None); cooldown_coins[symbol] = time.time() + 2*3600
                    await save_bot12_balance(client); await save_paper_trades(client); await save_watchlist(client)
                    asyncio.create_task(send_telegram(client, f"📈 <b>ST FLIP EXIT Runner</b> {symbol} #1/3 PnL ${nusdt:.2f} Watchlist Clear"))
                    return
            except: pass
...
async def bot1_scan(client):
    ...
    print("Bot1: Started v8.8.12 FIXED", flush=True)
    while True:
        try:
    ...
    try:
        ...
        async with _lock:
            ...
            if att == 0:
                ...
                cross_ok = (st_closed < ema_closed) and (close_closed < st_closed)
...
async def bot2_scan(client):
    print("Bot2: Started v8.8.12 FIXED", flush=True)
    while True:
        try:
...
async def check_bot3_paper_trades(client):
    try:
        ...
        for sym, trade in open_items:
            ...
            icon='✅' if net>=0 else '❌'
            await send_telegram(client, f"{icon} <b>BOT3 {reason} V8.8.12 SAME</b> {sym} {side}\nExit: ₹{exit_price:.2f}\nPnL: ₹{net:.2f}")
    except Exception as e: print(f"Bot3 trade manager error: {e}", flush=True)

async def bot3_nse_orb_async(client):
    print("Bot3 NSE ORB: Started v8.8.12 SAME AS V8.8.9 - 7 WINNERS 1%/1% EOD 3:00-3:10 PM", flush=True)
                ...
                if not ORB_LEVELS and 9 <= now_ist.hour <= 10:
                    if now_ist.minute % 10 in [1,2]:
                        ...
                        if temp:
                            ...
                            await send_telegram(client, f"📊 <b>NSE ORB READY v8.8.12</b> {len(ORB_LEVELS)}/{len(STOCKS_NSE)} WINNERS | 1% TP/SL | EOD 3:00-3:10 PM")
                if ORB_LEVELS:
                    ...
                    if is_market_window:
                        for sym, lv in list(ORB_LEVELS.items()):
                            ...
                            if close_price > lv['high'] and close_price > vwap:
                                ...
                                await send_telegram(client, f"🚀 <b>BOT3 LONG V8.8.12</b> {sym}\nEntry ₹{entry:.2f} TP ₹{tp:.2f} SL ₹{sl:.2f}")
                                ...
                                async with _lock:
                                    ...
                                    NSE_SIGNALS_TODAY.add(sym)
                                await save_nse_paper_trades(client)
                                await send_telegram(client, f"🔻 <b>BOT3 SHORT V8.8.12</b> {sym}\nEntry ₹{entry:.2f} TP ₹{tp:.2f} SL ₹{sl:.2f}")
                                ORB_LEVELS.pop(sym, None)
                    ...
                    if open_syms:
                        ...
                        for s in open_syms:
                            try:
                                ...
                                await send_telegram(client, f"⏰ <b>BOT3 EOD 3:00 PM V8.8.12</b>\n{s} {side}\nEntry ₹{entry:.2f} Exit ₹{price:.2f}\nPnL ₹{net:.2f} ({cond})")
                                await asyncio.sleep(0.3)
                            except Exception as e:
                                print(f"BOT3 EOD 15:00 error {s}: {e}", flush=True); failed_syms.append(s)
                        await save_bot3_balance(client); await save_nse_paper_trades(client)
                        if closed_count>0:
                            await send_telegram(client, f"🇮🇳 <b>BOT3 EOD SUMMARY 3:00 PM V8.8.12</b>\nClosed: {closed_count} trades\nTotal PnL ₹{total_pnl:.2f}\nFinal Bal ₹{BOT3_BALANCE_DATA['total_balance']:.2f}")
                        if failed_syms and now_min >= EOD_END-1:
                            await send_telegram(client, f"⚠️ EOD WARNING V8.8.12 Failed: {','.join(failed_syms)} - will retry till 15:10")
                    if now_min >= EOD_END:
                        if ORB_LEVELS:
                            await send_telegram(client, f"📊 <b>NSE EOD V8.8.12 3:00 PM</b> ORB left {len(ORB_LEVELS)} cleared")
...
@app.route('/')
def home(): return jsonify({"status":"v8.8.12 EMA300+SMA9 TP No #2 Runner -1% ST Exit BOT3 SAME","watchlist":len(WATCHLIST),"nse_orb":len(ORB_LEVELS),"bot12_bal":BOT12_BALANCE_DATA['total_balance'],"bot3_bal":BOT3_BALANCE_DATA['total_balance']})
...
async def main_async():
    ...
    async with httpx.AsyncClient(limits=limits) as client:
        ...
        print("v8.8.12 Operational - EMA SMA9 + TP No #2 + Runner -1% ST Exit", flush=True)
        try:
