...
def save_balance_data():
    global BALANCE_DATA
    gist_set('total_pnl.json', BALANCE_DATA)
...
def check_paper_trades(df, symbol):
    ...
    if current_price <= trade['tp'] or current_price >= trade['sl']:
        ...
        trade['pnl_percent'] = round(trade_pnl_percent, 2)
        trade['pnl_usdt'] = round(trade_pnl_usdt, 2)
        ...
        msg = f"{'✅' if hit_tp else '❌'} <b>PAPER TRADE CLOSED</b> {'✅' if hit_tp else '❌'}\n\n" \
              ...
              f"<b>Trade Amount:</b> ${trade_amount:.2f} (10% of Balance)\n" \
              f"<b>Trade PnL:</b> {trade_pnl_percent:.2f}% / ${trade_pnl_usdt:.2f}\n" \
                ...
                if new_cross:
                    ...
                    if cross_count < 3 and symbol not in PAPER_TRADES:
                        ...
                        msg = f"📝 <b>PAPER SHORT ENTRY</b> 📝\n\n" \
                              ...
                              f"<b>Trade Amount:</b> ${trade_amount:.2f} (10%)\n" \
                              f"<b>TP 5%:</b> ${tp_price}\n" \
                              f"<b>SL 2%:</b> ${sl_price}\n" \
                              f"<b>Balance:</b> ${BALANCE_DATA['total_balance']:.2f}"
                        send_telegram(msg)
