# COINDEX V8.8.3 - NSE DIRECT API FIX - NO yfinance - V8.8.2 based
...
async def start_command(u,c): await u.message.reply_text("✅ Bot v8.8.3 DIRECT NSE API - 3.5% SL + NO #3 + /nseorb", parse_mode="HTML")
...
async def help_command(u,c): await u.message.reply_text("📋 <b>V8.8.3 DIRECT NSE</b>\nSL 3.5% | No #3 | <code>/watchlist</code> <code>/nseorb</code>", parse_mode="HTML")

# --- NEW DIRECT NSE API - NO yfinance ---
...
async def get_nse_last_direct(client, symbol):
    try:
        ...
        ts_list = res.get('timestamp',[]); quote = res.get('indicators',{}).get('quote',[{}])[0]
        highs = quote.get('high',[]); lows = quote.get('low',[]); closes = quote.get('close',[]); vols = quote.get('volume',[])
        if not price:
            if closes and closes[-1]: price = closes[-1]
        if price and highs:
            tp_sum=0; vol_sum=0
...
async def nseorb_command(u,c):
    global ORB_LEVELS
    await u.message.reply_text("⏳ NSE ORB direct API V8.8.3... 20 stocks ~6 sec", parse_mode="HTML")
    cl = c.bot_data.get("http_client")
    ORB_LEVELS.clear()
    count=0; failed=[]
    ...
    msg = f"📊 <b>NSE ORB READY V8.8.3 DIRECT {count}/{len(STOCKS_NSE)}</b>\n"
    if ORB_LEVELS:
        for k,v in list(ORB_LEVELS.items())[:10]: msg+=f"{k} H:{v['high']:.1f} L:{v['low']:.1f}\n"
    if failed: msg+=f"\nFailed: {','.join(failed[:5])}"
    await u.message.reply_text(msg, parse_mode="HTML")

#... (rest same as v8.8.2, only bot3 uses direct)
    ...
    try:
        ...
        if chigh >= trade['sl']:
            ...
            async with _lock:
                ...
                if attempt == 1:
                    ...
                    if ENABLE_ATTEMPT_3: WATCHLIST[symbol]['attempts'] = 2; WATCHLIST[symbol]['last_state'] = 'wait_above_st'; WATCHLIST[symbol]['trigger_low'] = None; rmsg = f"❌ <b>SL HIT</b> {symbol} #{attempt}/3 ⏳ Waiting ST above LIVE"
        ...
        if clow <= trade['tp']:
            if attempt == 1 and not trade.get('tp1_hit'):
                partial = amt_orig * 0.5; gpct = ((entry - trade['tp']) / entry) * 100; gusdt = partial * gpct / 100; fee = partial * EFFECTIVE_FEE_RATE + (partial + gusdt) * EFFECTIVE_FEE_RATE; nusdt = gusdt - fee
                async with _lock:
                ...
                amt = amt_orig * (0.5 if trade.get('tp1_hit') else 1.0); gpct = ((entry - trade['tp']) / entry) * 100; gusdt = amt * gpct / 100; fee = amt * EFFECTIVE_FEE_RATE + (amt + gusdt) * EFFECTIVE_FEE_RATE; nusdt = gusdt - fee; npct = (nusdt / amt) * 100 if amt > 0 else 0
                async with _lock:
                    BALANCE_DATA['total_balance'] += nusdt; BALANCE_DATA['lifetime_pnl_usdt'] = BALANCE_DATA['total_balance'] - BALANCE_DATA['starting_balance']; BALANCE_DATA['lifetime_pnl_percent'] = (BALANCE_DATA['lifetime_pnl_usdt'] / BALANCE_DATA['starting_balance']) * 100
...
async def bot1_scan(client):
    print("Bot1: Started v8.8.3", flush=True)
    while True:
        try:
        ...
        async with _lock:
            ...
            if att == 0:
                ...
                if trig is None:
                    ...
                    if is_cross: WATCHLIST[symbol]['trigger_low'] = low_closed; WATCHLIST[symbol]['last_state'] = 'waiting_break_1'; changed = True; asyncio.create_task(send_telegram(client, f"📌 <b>1st Trigger Marked</b> {symbol} Low ${low_closed:.8f}"))
                else:
                ...
                if WATCHLIST[symbol].get('trigger_low') is None:
                    ...
                    if is_cross: WATCHLIST[symbol]['trigger_low'] = low_closed; WATCHLIST[symbol]['last_state'] = 'waiting_break_2'; changed = True; asyncio.create_task(send_telegram(client, f"📌 <b>2nd Trigger Marked</b> {symbol} Low ${low_closed:.8f}"))
                else:
                    trig = WATCHLIST[symbol]['trigger_low']
                    if low_live <= trig - (tick * TRIGGER_TICKS): should = True; trig_for_msg = trig; exec_price = (trig - (tick * TRIGGER_TICKS)) * (1 - SLIPPAGE_PCT)
            elif att == 2:
                if not ENABLE_ATTEMPT_3: WATCHLIST.pop(symbol, None); cooldown_coins[symbol] = time.time() + 3*3600; changed = True; return changed
                state = WATCHLIST[symbol].get('last_state', 'wait_above_st')
                if state == 'wait_above_st' and live_price_for_check > st_live and st_dir_live == -1: WATCHLIST[symbol]['last_state'] = 'ready_for_st_cross'; WATCHLIST[symbol]['trigger_low'] = None; changed = True; asyncio.create_task(send_telegram(client, f"📈 <b>Above ST LIVE</b> {symbol} Live ${live_price_for_check:.8f} > ST ${st_live:.8f} - Ready for #3"))
...
async def bot2_scan(client):
    print("Bot2: Started v8.8.3", flush=True)
    while True:
        try:
...
async def bot3_nse_orb_async(client):
    print("Bot3 NSE ORB: Started v8.8.3 DIRECT", flush=True)
                ...
                if not ORB_LEVELS and 9 <= now_ist.hour <= 10:
                    if now_ist.minute % 10 in [1,2]:
                        ...
                        if temp:
                            ...
                            await send_telegram(client, f"📊 <b>NSE ORB READY v8.8.3 DIRECT</b> {len(ORB_LEVELS)}/{len(STOCKS_NSE)} stocks\nUse /nseorb to recalc")
                if ORB_LEVELS:
...
@app.route('/')
def home(): return jsonify({"status":"v8.8.3 DIRECT NSE","watchlist":len(WATCHLIST),"cooldown":len(cooldown_coins),"nse_orb":len(ORB_LEVELS)})
...
async def main_async():
    ...
    async with httpx.AsyncClient(limits=limits) as client:
        ...
        print("v8.8.3 DIRECT NSE Operational", flush=True)
        try:
